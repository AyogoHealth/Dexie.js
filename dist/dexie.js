   ...
   function Dexie(dbName, options) {
       ...
       extend(Version.prototype, {
           ...
           stores: function (stores) {
               ...
               for (var i = 0, ii = versions.length; i < ii; i++) {
                   ...
                   if (version === this) {
                       var undeleted = filter(version._cfg.storesSource, function (v) {
                           return v !== null;
                       });
               ...
               this._cfg.preUpgradeDbSchema = this._parseStoresSpec(preUpgradeStoresSpec);
               this._cfg.postUpgradeDbSchema = this._parseStoresSpec(postUpgradeStoresSpec);

               var schema = filter(this._cfg.postUpgradeDbSchema, function (v) {
                   return v !== null;
               });
               setApiOnPlace([this._cfg.tables], keys(schema), READWRITE, schema);
               this._update();
           ...
           upgrade: function (upgradeFunction) {
               ...
               fakeAutoComplete(function () {
                   ...
                   var schema = _this._cfg.preUpgradeDbSchema;
           ...
           _schema: function () {
               return this._cfg.postUpgradeDbSchema;
           },
           ...
           _update: function () {
               ...
               if (db.verno === this._cfg.version) {
                   ...
                   var schema = filter(this._schema(), function (v) {
                       return v !== null;
                   });
                   setApiOnPlace([allTables, db, Transaction.prototype], keys(schema), READWRITE, schema);
                   globalSchema = db._dbSchema = schema;
               } else {
                   // Update higher versions recursively.
                   var next = versions.find(function (v) {
                       return v._cfg.version > _this2._cfg.version;
                   });
                   next.stores({});
               }
           }
       ...
       extend(Version, {
           ...
           diff: function (a, b) {
               ...
               for (var table in a) {
                   if (!b[table]) diffs.del.push(table);
               }
               for (var _table in b) {
                   var oldDef = a[_table],
                       newDef = b[_table];
           ...
           filterSchema: function (s) {
               return filter(s, function (s) {
                   return s !== null;
               });
           }
       });
       ...
       function runUpgraders(oldVersion, idbtrans, reject) {
           var activeSchema = filter(globalSchema, function (s) {
               return s !== null;
           });
           var trans = new Transaction(READWRITE, keys(activeSchema), activeSchema);
           ...
           newScope(function () {
               ...
               // Initial creation.
               if (oldVersion === 0) {
                   for (var name in globalSchema) {
                       var spec = globalSchema[name];
                       createTable(idbtrans, name, spec.primKey, spec.indexes);
                   }
       ...
       function updateTablesAndIndexes(oldVersion, trans, idbtrans) {
           ...
           var queue = [];
           var oldVersionStruct = versions.find(function (version) {
           ...
           versToRun.forEach(function (version) {
               ...
               queue.push(function () {
                   ...
                   adjustToExistingIndexNames(version._upgradeSchema(), idbtrans);

                   diff = Version.diff(lastSchema, newSchema);
                   lastSchema = newSchema;
                   // Add tables           
                   diff.add.forEach(function (tableschema) {
                       createTable(idbtrans, tableschema.name, tableschema.primKey, tableschema.indexes);
                   });
                   // Change tables
               ...
               queue.push(function (idbtrans) {
                   ...
                   if (!anyContentUpgraderHasRun || !hasIEDeleteObjectStoreBug) {
                       diff.del.forEach(function (name) {
       ...
       */
       ...
       derive(WriteableTable).from(Table).extend({
           ...
           bulkPut: function (objects, keys) {
               ...
               return this._idbstore(READWRITE, function (resolve, reject, idbstore) {
                   if (!idbstore.keyPath && !_this3.schema.primKey.auto && !keys) throw new exceptions.InvalidArgument("bulkPut() with non-inbound keys requires keys array in second argument");
                   ...
                   var done = function (result) {
                       if (errorList.length === 0) resolve(result);else reject(new BulkError(_this3.name + '.bulkPut(): ' + errorList.length + ' of ' + numObjs + ' operations failed', errorList));
                   };
                   var req,
                       errorList = [],
                       errorHandler,
                       ...
                       table = _this3;
                   if (_this3.hook.creating.fire === nop && _this3.hook.updating.fire === nop) {
                       //
       ...
       function Transaction(mode, storeNames, dbschema, parent) {
           ...
           this._completion.then(function () {
               _this4.on.complete.fire();
           }, function (e) {
               _this4.on.error.fire(e);
               _this4.parent ? _this4.parent._reject(e) : _this4.active && _this4.idbtrans && _this4.idbtrans.abort();
               _this4.active = false;
       ...
       props(Transaction.prototype, {
           ...
           create: function (idbtrans) {
               ...
               idbtrans.onerror = wrap(function (ev) {
                   ...
                   _this5._reject(idbtrans.error);
               });
               idbtrans.onabort = wrap(function (ev) {
                   preventDefault(ev);
                   _this5.active && _this5._reject(new exceptions.Abort());
                   _this5.active = false;
                   _this5.on("abort").fire(ev);
               });
           ...
           tables: {
               get: function () {
                   ...
                   return this._table_cache = arrayToObject(this.storeNames, function (name) {
                       return [name, _this6._tables[name]];
                   });
               }
           },
   ...
   props(TableSchema, {
       ...
       diff: function (a, b) {
           ...
           if (a.primKey.src !== b.primKey.src) {
               ...
               for (var _idxName in newIndexes) {
                   var oldIdx = oldIndexes[_idxName],
                       newIdx = newIndexes[_idxName];
                   if (!oldIdx) {
